    ...
    <div style={styles.loginWrap}>
      <div style={styles.loginCard}>
        ...
        <form onSubmit={handleLogin} style={{width:'100%'}}>
          <input type="password" placeholder="Slaptažodis" value={pw}
            onChange={e => setPw(e.target.value)} style={styles.loginInput} autoFocus/>
          ...
          {ALL_TYPES.map(t => {
            ...
            return (
              <button key={t} onClick={() => onSelect(t)} style={{
                ...
                outline: t === currentType ? `2px solid ${ts.color}` : 'none',
              }}>
                {TYPE_LABEL[t]} {t === currentType ? '✓' : ''}
              </button>
            );
          })}
        </div>
        <button onClick={onClose} style={{...styles.backBtn, marginTop:12, width:'100%', textAlign:'center'}}>Atšaukti</button>
      </div>
    </div>
  );
}

function Calendar({ workouts, dayTypes, onDayClick }) {
  const getType = (d) => dayTypes['2025-06-' + String(d).padStart(2,'0')] || DEFAULT_SCHEDULE[d] || 'rest';
  const isLogged = (d) => {
  ...
  return (
    <div style={styles.calGrid}>
      {DAY_SHORT.map(d => <div key={d} style={styles.calHeader}>{d}</div>)}
      {Array.from({length:6}, (_,i) => <div key={'e'+i} style={styles.emptyCell}/>)}
      {Array.from({length:30}, (_,i) => {
        ...
        return (
          <div key={d} onClick={() => onDayClick(d)} style={{
            ...styles.dayCell, background:ts.bg, border:`1px solid ${ts.border}`, cursor:'pointer',
          }}>
            <span style={styles.dayNum}>{d}</span>
            <span style={{...styles.dayType, color:ts.color, fontSize:type==='match'?10:11}}>{TYPE_LABEL[type]}</span>
            {logged && <span style={styles.logged}>✓</span>}
          </div>
        );
      })}
    </div>
...
function DayView({ day, initial, currentType, onBack, onSave, onChangeType }) {
  ...
  const [dayData, setDayData] = useState(() => {
    ...
    return makeEmptyDay(currentType);
  });
  ...
  async function handleTypeChange(newType) {
    ...
    await onChangeType(dateKey, newType);
  ...
  return (
    <div style={styles.dayView}>
      {showTypePicker && (
        <TypePicker currentType={currentType} onSelect={handleTypeChange} onClose={() => setShowTypePicker(false)}/>
      )}
      ...
      <div style={{...styles.dayHeader, background:ts.bg, borderColor:ts.border}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            ...
            <div style={{...styles.dayBadge, color:ts.color}}>{TYPE_LABEL[currentType]}</div>
          </div>
          <button onClick={() => setShowTypePicker(true)} style={{
            background:'rgba(255,255,255,0.7)', border:`1px solid ${ts.border}`,
            borderRadius:8, padding:'5px 10px', fontSize:12, color:ts.color,
      ...
      {dayData.blocks.map((block, bi) => (
        <div key={bi} style={styles.blockCard}>
          <div style={styles.blockTitle}>
            {block.name}{block.params ? <span style={styles.blockParams}> · {block.params}</span> : null}
          </div>
          ...
          {block.exercises.map((ex, ei) => (
            <div key={ei} style={styles.exRow}>
              <input style={{...styles.input,flex:1}} value={ex.name} placeholder="Pratimas..."
                onChange={e => updateEx(bi, ei, 'name', e.target.value)}/>
              <input style={{...styles.input,...styles.inputNum}} type="number" min="1" max="20"
                value={ex.sets} placeholder="—" onChange={e => updateEx(bi, ei, 'sets', e.target.value)}/>
              <input style={{...styles.input,...styles.inputNum}} type="number" min="1" max="100"
                value={ex.reps} placeholder="—" onChange={e => updateEx(bi, ei, 'reps', e.target.value)}/>
              <input style={{...styles.input,...styles.inputNum}} type="number" min="0" step="0.5"
                value={ex.weight} placeholder="—" onChange={e => updateEx(bi, ei, 'weight', e.target.value)}/>
      ...
      <div style={styles.blockCard}>
        ...
        <textarea style={styles.textarea} placeholder="Kaip jautėtės, nuovargis..."
          value={dayData.note || ''}
...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [w, dt] = await Promise.all([
        apiReq('GET', '/api/workouts'),
        apiReq('GET', '/api/daytypes'),
      ]);
  ...
  async function handleChangeType(dateKey, newType) {
    try {
      await apiReq('PUT', '/api/daytypes/' + dateKey, { type: newType });
      setDayTypes(prev => ({...prev, [dateKey]: newType}));
    } catch {}
  }

  function getTypeForDay(d) {
    const dateKey = '2025-06-' + String(d).padStart(2,'0');
    return dayTypes[dateKey] || DEFAULT_SCHEDULE[d] || 'rest';
  }

  if (!loggedIn) return <LoginScreen onLogin={() => setLoggedIn(true)} />;
  if (loading) return <div style={styles.loadingWrap}><div style={styles.loadingText}>Kraunama...</div></div>;
    ...
    return (
      <div style={styles.app}>
        <DayView
          ...
          currentType={getTypeForDay(activeDay)}
  ...
  return (
    <div style={styles.app}>
      <div style={styles.topBar}>
        ...
        <button onClick={() => { clearToken(); setLoggedIn(false); }} style={styles.logoutBtn}>Atsijungti</button>
      </div>
      ...
      <Calendar workouts={workouts} dayTypes={dayTypes} onDayClick={setActiveDay}/>
      <div style={styles.footer}>{Object.keys(workouts).length} treniruočių įrašyta · Spausk ant dienos kad keistum tipą ar įrašytum</div>
    </div>
  );
}
